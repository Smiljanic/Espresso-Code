...
//*************************************
//MILENA: on creation rigid aggregates...
//Take orientation from particle 1 and 2, and calculate their mutual orientation
  double orientationp1[3], orientationp2[3], vecpro[3], vec2[3]; 

  orientationp1[0]=p1->r.quatu[0];
  orientationp1[1]=p1->r.quatu[1];
  orientationp1[2]=p1->r.quatu[2];

  orientationp2[0]=p2->r.quatu[0];
  orientationp2[1]=p2->r.quatu[1];
  orientationp2[2]=p2->r.quatu[2];

  double mutual_angle = scalar(orientationp1, orientationp2);
  vector_product(orientationp1, orientationp2, vecpro);
 // double vecpro_magnitude = sqrln(vecpro);
 // double vecpro_inv = 1.0 / sqrt(vecpro_magnitude);
 // for(int j=0;j<3;j++) vecpro[j] *= vecpro_inv;

  get_mi_vector(vec2, p1->r.p, p2->r.p);
  
  //The triangle: vector (vecpro) perpendicular to the ellipsoids directors (orientationp1, orientationp2) 
  //passes through the point_for_additional_vs at the middle of connecting vector (vec2)
  double point_for_additional_vs[3];
  point_for_additional_vs[0] = 0.5*(p1->r.p[0]-p2->r.p[0]);
  point_for_additional_vs[1] = 0.5*(p1->r.p[1]-p2->r.p[1]);
  point_for_additional_vs[2] = 0.5*(p1->r.p[2]-p2->r.p[2]);

  double normal_to_connecting_vector = scalar(vec2, orientationp1);
  
  double up_left_point[3], up_middle_point[3], up_right_point[3];
  



  up_left_point[0] = normal_to_connecting_vector[0]+0.5*sin(PI/4);
 // up_left_point[1] = point_for_additional_vs[1];
 // up_left_point[2] = point_for_additional_vs[2] + pow(2.0, 1./6.);
  
  up_middle_point[0] = point_for_additional_vs[0];
 // up_middle_point[1] = point_for_additional_vs[1];
 // up_middle_point[2] = point_for_additional_vs[2] + pow(2.0, 1./6.);

  up_right_point[0] = point_for_additional_vs[0]-0.5*sin(PI/4);
 // up_right_point[1] = point_for_additional_vs[1];
 // up_right_point[2] = point_for_additional_vs[2] + pow(2.0, 1./6.);

   // normalize
 // dist2 = sqrlen(vec2);
 // double d_inv = 1.0 / sqrt(dist2);
 // for(int j=0;j<3;j++) vec2[j] *= d_inv;

  //printf("vector product %d %d %d ", vecpro[0],vecpro[1], vecpro[2]);

  //normalize the vector product
  

//TRACE(printf("Vector product of particle orientation %f %f %f\n",vecpro[0], vecpro[1], vecpro[2])); 
  //TRACE(printf("Orientation of the particle 1 : %f %f %f\n",orientationp1[0],orientationp1[1],orientationp1[2]));
...
void handle_collisions ()
{
  ...
  gather_collision_queue(counts);
    ...
    } // If we have domain decomposition
