...
//***********************************
//On ellipsoid collision and making irreversible bonds
  ...
  //double vecpro[3];
  //vector_product(orientation_p1, orientation_p2, vecpro); 
  ...
  double middle_point_at_connecting_vector[3]; //(x0,y0,z0) point on the plane
  middle_point_at_connecting_vector[0] = 0.5*(p2->r.p[0]+p1->r.p[0]);
  middle_point_at_connecting_vector[1] = 0.5*(p2->r.p[1]+p1->r.p[1]);
  middle_point_at_connecting_vector[2] = 0.5*(p2->r.p[2]+p1->r.p[2]);

  // normal plane: a(x-x0)+b(y-y0)+c(z-z0)=0
  // connecting_vector[0]*(x-middle_point_at_connecting_vector[0]) + 
  // connecting_vector[1]*(y-middle_point_at_connecting_vector[1]) + 
  // connecting_vector[2]*(z-middle_point_at_connecting_vector[2]) = 0 

  double left_point[3], up_middle_point[3], right_point[3]; 
  //up_left_point: moved in z direction for e, x the same, y from the equation:
  //up_right_point: moved in z direction for -e, x the same, y from the equation:
  //up_middle_point: moved in x for e, y the same, z for the equation
 
  left_point[0]=middle_point_at_connecting_vector[0];
  left_point[2]=middle_point_at_connecting_vector[2]+0.5*pow(2.0,1.0/6.0);
  left_point[1]=middle_point_at_connecting_vector[1]+(-connecting_vector[2]/connecting_vector[1])*(left_point[2]-middle_point_at_connecting_vector[2]);
   
  right_point[0]=middle_point_at_connecting_vector[0];
  right_point[2]=middle_point_at_connecting_vector[2]-0.5*pow(2.0,1.0/6.0);
  right_point[1]=middle_point_at_connecting_vector[1]+(-connecting_vector[2]/connecting_vector[1])*(right_point[2]-middle_point_at_connecting_vector[2]);
   
  
  //up_middle_point[0]=middle_point_at_connecting_vector[0]+(-connecting_vector[1]/connecting_vector[0])*(up_middle_point[1]-middle_point_at_connecting_vector[1]);
  //up_middle_point[2]=middle_point_at_connecting_vector[2];
  //up_middle_point[1]=middle_point_at_connecting_vector[1]-0.5*pow(2.0,1.0/6.0);

  up_middle_point[0]=middle_point_at_connecting_vector[0]+0.5*pow(2.0,1.0/6.0);  
  up_middle_point[1]=middle_point_at_connecting_vector[1];  
  up_middle_point[2]=middle_point_at_connecting_vector[2]+(connecting_vector[0]/connecting_vector[2])*(up_middle_point[0]-middle_point_at_connecting_vector[2]);  
  

  double vector[3];
  get_mi_vector(vector, up_middle_point, middle_point_at_connecting_vector);
  
  double check;
  check = scalar(connecting_vector,vector);

  //printf("particle id %d and %s\d", p1->p.identity, p2->p.identity);
//returns the following error:::
/*WARNING: The behavior of the Langevin thermostat has changed
         as of this version! Please consult the user's guide.
[nusselt:07683] *** Process received signal ***
[nusselt:07683] Signal: Segmentation fault (11)
[nusselt:07683] Signal code: Address not mapped (1)
[nusselt:07683] Failing at address: 0x1
[nusselt:07683] [ 0] /lib/x86_64-linux-gnu/libpthread.so.0(+0x10340) [0x7fe81ec38340]
[nusselt:07683] [ 1] /lib/x86_64-linux-gnu/libc.so.6(_IO_vfprintf+0x1d13) [0x7fe81e8aba03]
[nusselt:07683] [ 2] /lib/x86_64-linux-gnu/libc.so.6(_IO_printf+0x99) [0x7fe81e8b4499]
[nusselt:07683] [ 3] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x45594d]
[nusselt:07683] [ 4] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x4692d2]
[nusselt:07683] [ 5] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x4bc4b0]
[nusselt:07683] [ 6] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x476118]
[nusselt:07683] [ 7] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x47494c]
[nusselt:07683] [ 8] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x459ad5]
[nusselt:07683] [ 9] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x40d81d]
[nusselt:07683] [10] /usr/lib/x86_64-linux-gnu/libtcl8.6.so(TclInvokeStringCommand+0x80) [0x7fe81fc51070]
[nusselt:07683] [11] /usr/lib/x86_64-linux-gnu/libtcl8.6.so(TclNRRunCallbacks+0x47) [0x7fe81fc53337]
[nusselt:07683] [12] /usr/lib/x86_64-linux-gnu/libtcl8.6.so(+0x3c30f) [0x7fe81fc5430f]
[nusselt:07683] [13] /usr/lib/x86_64-linux-gnu/libtcl8.6.so(Tcl_FSEvalFileEx+0x21f) [0x7fe81fcfb4ef]
[nusselt:07683] [14] /usr/lib/x86_64-linux-gnu/libtcl8.6.so(Tcl_MainEx+0x2db) [0x7fe81fd0266b]
[nusselt:07683] [15] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x40c2a7]
[nusselt:07683] [16] /lib/x86_64-linux-gnu/libc.so.6(__libc_start_main+0xf5) [0x7fe81e881ec5]
[nusselt:07683] [17] ../../Espresso-Rudolf/espresso-gb/Espresso() [0x40c189]
[nusselt:07683] *** End of error message ***
Segmentation fault (core dumped)
itvms@nusselt:~/PHD/CODE/ellipsoids_gb_scripts/ellipsoids_in_cross$ ../../Espresso-Rudolf/espresso-gb/Espresso ellipsoid-collision-detection.tcl
*/
  printf("Scalar product of connecting_vector and vector in the plane is:  %f\n", check); 
  printf("Middle point %f %f %f and up_mid %f %f %f\n", middle_point_at_connecting_vector[0], middle_point_at_connecting_vector[1], middle_point_at_connecting_vector[2], up_middle_point[0], up_middle_point[1], up_middle_point[2]);
  printf("***********************************\n");
